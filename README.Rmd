...
behavior of arrow assignment operator replacement formatting rule so that 
whenever \code{<-} is present it is replaced with \code{=}.
...
Unfortunately, this is more of a style guideline change. Furthermore,
the API for the `tidy_source` function is already quite established and adding
an option to seeminly invert the authors intended functionality would 
yield numerous breakages. So, the safer course of action is to provide a way
for regenades of assignment to express themselves since:
